...
if [ -n "${DOWNLOAD_INDEX}" ]; then
    ...
    if [ "$JAVA6" == "sun-java6" ]; then
        ...
        wget "http://www.oracle.com/technetwork/java/javasebusiness/downloads/java-archive-downloads-javase7-521261.html" -O "/tmp/oab-download-$JAVA6.html" >> "$LOG" 2>&1 &
        pid=$!;progress $pid
    fi
fi
