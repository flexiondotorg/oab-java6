...
wget "http://www.oracle.com/technetwork/java/javase/downloads/index.html" -O /tmp/oab-index.html >> "$3" 2>&1 &
pid=$!;progress $pid
...
if [ -n "${DOWNLOAD_INDEX}" ]; then
    ...
    wget http://www.oracle.com/${DOWNLOAD_INDEX} -O /tmp/oab-download.html >> "$3" 2>&1 &
    pid=$!;progress $pid
else
    ncecho " [x] Failed to get current release page, getting previous releases download page "
    if [ "$4" == "sun-java6" ]; then
        wget http://www.oracle.com/technetwork/java/javasebusiness/downloads/java-archive-downloads-javase6-419409.html -O /tmp/oab-download.html >> "$3" 2>&1 &
        pid=$!;progress $pid
    else
        wget http://www.oracle.com/technetwork/java/javasebusiness/downloads/java-archive-downloads-javase7-521261.html -O /tmp/oab-download.html >> "$3" 2>&1 &
        pid=$!;progress $pid
    fi
fi
...
do
    ...
    wget -c "${DOWNLOAD_URL}" -O $2/pkg/${JAVA_BIN} >> "$3" 2>&1 &
    pid=$!;progress_loop $pid

    ncecho " [x] Symlinking ${JAVA_BIN} "
    ln -s $2/pkg/${JAVA_BIN} $2/src/$4/${JAVA_BIN} >> "$3" 2>&1 &
    pid=$!;progress_loop $pid    
done
