...
export DEB_VERSION=`head -n1 "$BASE/src/$1/debian/changelog" | cut -d'(' -f2 | cut -d')' -f1 | cut -d'~' -f1`
export DEB_URGENCY=`head -n1 "$BASE/src/$1/debian/changelog" | cut -d'=' -f2`
...
do
    ...
    ln -s "$BASE/pkg/${JAVA_BIN}" "$BASE/src/$1/${JAVA_BIN}" >> "$LOG" 2>&1 &
    pid=$!;progress_loop $pid    
done
